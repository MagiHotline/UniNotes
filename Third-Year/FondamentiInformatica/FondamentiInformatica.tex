...
\thm{}
{
   $\forall n,m \ge 1$ esiste una funzione totale ricorsiva $S$ tale che $\forall x, y_1, y_2, \dots, y_m \in \mathbb{N}$:
   \[  
    \lambda z_1 \dots z_n \; . \; \underbrace{\varphi_x}_{\text{codice}}(\overbrace{y_1, y_2, \dots, y_m}^{\text{input che specializziamo}}, \underbrace{z_1, z_2, \dots, z_n}_{\text{input che rimane incognito}}) = \varphi_{S(x,y_1,y_2,\dots,y_m)}(z_1,z_2,\dots,z_n)
   \]
   Quindi abbiamo una trasformazione totale $S$ del codice $x$ e considerando gli input specializzati $y_1, \dots, y_m$.
}

\begin{proof}
  $m = n = 1$ (caso particolare ma $m,n > 1$ è analogo)
  \[
  ...
  \]
  dove $\varphi_{x_0}$ denota la funzione calcolata della MdT il cui codice è aritmetizzato in x. Questa funzione è intuitivamente calcolabile.
  Di conseguenza esiste una MdT che la calcola per la tesi di Church il cui codice dipende 
  ed è costruibile ricorsivamente a partire da $x_0$ e $y_0$.
  Quindi esiste $S$ totale ricorisivan tale che il codice è $S(x_0,y_0)$.
  \[
...
Dato un programma sorgente $source \in L$ esiste una trasformazione 
di $source$ in un programma (semanticamente) equivalente $target \in L'$.


\begin{proof}
  Supponiamo $Int$ sia la macchina universale.
  \[
    Int(p, d) = P(d)
  \]
  Supponaniamo $spec$ sia uno specializzatore, dove $Q$ è un programma a due input
  \[
...
\subsubsection{Linguaggi Turing completi}

  Un linguaggio di programmazione è Turing completo se
  permette di calcolare tutte le funzioni intuitivamente calcolabili.
  \[
  \begin{aligned}
    ...
    &\exists spec . \varphi_{spec(xy)(z)} = \lambda z . \varphi_x (y,z) \text{ specializzazione}
  \end{aligned}
  \]
  Se consideriamo la primitiva ricorsione abbiamo solo cicli $for$. Un linguaggio che non permette 
  cicli non determinati \textbf{non} è Turing completo. 
  \[
    \exists \; MdT \; m \; . \; L = L(M) = \{y \; | \; \varphi_m(y) \downarrow \}
  \]
  Quindi in linguaggi di Turing sono linguaggi accettati da una macchina di Turing.
  $y$ sono le stringhe su cui la MdT termina.
  Se noi abbiamo $\varphi_x$ parziale ricorsiva ovvero 
  calcolata da una MdT dal codice $x$ allora definiamo 
  \[
    Dom(\varphi_x) = \{y \; | \; \varphi_x(y) \downarrow \}
  \]
...
La teoria della ricorsione parla degli insiemi (linguaggi)
caratterizzati dalle MdT. \textbf{Ricorsivamente enumerabile} 
è un insieme di elementi (numeri) costruibili in modo algoritmico.
\dfn{Insieme ricorsivamente enumerabile (RE)}
{
  Un insieme $I \subseteq \mathbb{N}$ 
  è recorsivamente enumerabile se è il dominio di una funzione parziale ricorsiva,
  \[
    \exists \varphi_x \text{ parziale ricorsiva } . \; dom(\varphi_x) = I
  \]
}
\noindent
Da qui in poi useremo la notazione dove $W_x \stackrel{def}{=} dom(\varphi_x)$.
Se abbiamo $\varphi_x$ parziale ricorsiva allora $W_x$ è ricorsivamente enumerabile.
Se abbiamo $I \subseteq \mathbb{N}$ e riusciamo a costruire la sua  
funzione semicaratteristica
\[
  \psi_I(x) = \begin{cases}
    1 & x \in I\\
    \uparrow & \text{altrimenti}
  \end{cases} \implies dom (\psi_I) = I
\]
Per dimostrare che un insieme è ricorsivamente enumerabile 
dobbiamo fornire l'algoritmo della semicaratteristica
che deve terminare se l'input appartiene all'insieme.

\ex{Problema della terminazione (variante)}
{
  \[\{x \; | \; \exists y \; . \; \varphi_x(y)\downarrow\}\]
  Questo programma cerca di determinare i programmi che terminan 
  su almeno un input. Si può riscrivere con il seguente insieme:
  \[
    \{x \; | \; \varphi_x(y)\downarrow\}
  \]
  \[
    \begin{cases}
      input(x)\\
      stop = 0;\\
      build \; \varphi_x \text{ esiste algoritmo ricorsivo}\\
      \text{while } \neg \; stop \text{ do }\\
       \quad \text{ Execute next step } \varphi_x(0)\\
       \quad \text{if } \varphi_x(0) \downarrow \text{ then stop = 1}\\
      \text{return 1}
    \end{cases}
  \]
  La funzione è semicaratteristica perché se $\varphi_x(0)$ restituisce 1 altrimenti diverge. 
  \[
  I = \{x \; | \; \exists y \; . \; \varphi_x(y) \downarrow \}
  \]
  \[\begin{cases}
    input(x)\\
    y = 0\\
    stop = 0\\
    \text{while } \neg \; stop \text{ do }\\
       \quad \text{ Execute next step } \varphi_x(y)\\
       \quad \text{if } \varphi_x(0) \downarrow \text{ then stop = 1}\\
    \text{return 1}
  \end{cases}\]
  Se diverge su $0$ non potrò mai testare $y > 0$ ma $\varphi_x(1)$ potrebbe terminare.
  Questo algoritmo diverge per $x \in I$ quindi questo algoritmo NON è la semicaratteristica. 
  \begin{itemize}
    \item Passo 0: Esegue un passo di $\varphi_x(0)$ se $\downarrow$ allora fine, se non termino eseguiamo il passo successivo 
    \item Passo $n+1$:  Esegue un passo di $\varphi_x(0), \varphi_x(1), \dots, \varphi_x(n)$ (se mai sono terminate prima) esegui il primo passo per $\varphi_x(n+1)$
  \end{itemize}
  Questo algoritmo si chiama \textbf{Dovetail}.
  La semicaratteristica per $I$ quindi:
  \[
    \begin{cases}
      input(x)\\
      \text{costruiamo } \varphi_x \text{ (esiste una procedura ricorsiva per costruire $\varphi_x$ da $x$)}\\
      y = 0 \text{ (serve per identicare gli input su cui abbiamo iniziato la computazione)}\\
      stop = 0\\
      \text{while } \neg \; stop \text{ do }\\
         \quad \text{for } i = 0 \text{ to } y \text{ do }\\
         \quad \quad \text{ Execute next step } \varphi_x(i)\\
         \quad \quad \text{ if } \varphi_x(i) \downarrow \text{ then stop = 1}\\
         \quad y = y + 1\\
      \text{return 1}
    \end{cases}
  \]
...
Esiste un algoritmo che decide se un
elemento (numero) appartiene o meno ad un insieme.
Nel caso degli insiemi ricorsivi, al contrario di quelli ricorsivamente enumerabili,
è che riesco a dire sia se un elemento appartiene che se non appartiene all'insieme.
\dfn{Insieme ricorsivo (R)}
{
  Un insieme $I \subseteq \mathbb{N}$ è ricorsivo se la sua \textbf{funzione caratteristica}
  \[
    \chi_I(x) = \begin{cases}
      1 & x \in I\\
      0 & x \notin I
...
\noindent
Per dimostrare che un insieme è ricorsivo, gli dobbiamo fornire l'algoritmo 
che calcola la sua funzione caratteristica.
\ex{}
{
  Per esempio:
  \[
  ...
  \begin{cases}
    input(x)\\
    \text{if } x \text{ mod } 2 = 0 \text{ then output(1)}\\
    \text{else output(0)}
  \end{cases}
  \]
...
\ex{}
{
  ...
  \[
    ...
    \end{cases}
  \]
  Questo programma \textbf{NON} è la caratteristica perché non può essere totale.
  \[
    \begin{cases}
      input(x)\\
      n = 0\\
      \text{while } n^2 \le x \text{ do }\\
        \quad \text{if } n^2 = x \text{ then output(1)}\\
        \quad n++\\
      output(0)
    \end{cases}
  \]
...
\begin{proof}
  Se $A$ è ricorsiva allora $A$ è RE. 
  Analogamente costruiamo la semicaratteristica per $\bar{A}$:
  \[
    f_{\bar{A}}(x) = \begin{cases}
      1 & \text{se } f_A(x) = 0\\
      \uparrow & \text{altrimenti}
    \end{cases}
  \] 
  ...
  \begin{cases}
    ...
    stop = 0\\
    \text{while } \neg \; stop \text{ do } \{\\
      \quad \text{Execute next step } \psi_A(x)\\
      \quad \text{if } \psi_A(x) \downarrow \text{ then } \{output(1); stop = 1\}\\
      \quad \text{Execute next step } \psi_{\bar{A}}(x)\\
      \quad \text{if } \psi_{\bar{A}}(x) \downarrow \text{ then } \{output(0); stop = 1\}\\
    \}   
  \end{cases}
  \]
\end{proof}
...
{
  \[
    A_m = \{x \; | \; \varphi_x(x) \downarrow \text{ in meno di $m$ passi}\}
  \]
  ...
  \[
    \begin{cases}
      input(x)\\
      \text{costruiamo } \varphi_x\\
      \text{for } i = 1 \text{ to } m \text{ do }\\
        \quad \text{ Execute next step } \varphi_x(x)\\
        \quad \text{if } \varphi_x(x) \downarrow \text{ then output(1)}\\
      output(0)
    \end{cases}
  \]
...
\noindent
Cosa abbiamo visto fino ad adesso: 
\begin{itemize}
  \item Per dimostrare che un insieme non è ricorsivo si usa il \textbf{Teorema di Rice}
  \item Per dimostrare che un insieme è RE si fornisce l'algoritmo che implementa la funzione semicaratteristica
  \item Per dimostare che un insieme non è RE si usa il \textbf{Teorema di Post}
  \[ 
    A \text{ è RE } \iff \bar{A} \text{ è RE}
  \]
\end{itemize}
...
Prima di parlare del teorema di Rice bisogna definire cosa vuol dire proprietà.

\ex{}
{
  Proprietà di essere pari: $x$ è pari se $x$ div $2$ $=$ resto $0$:
  \[  
    P = \{x \; | \; x \text{ mod }  2 = 0\}
  \]
  Questo è l'insieme di tutti e soli numeri pari. Rappresenta in modo 
  estensionale la proprietà di essere pari.
  Ogni sottoinsieme di numeri rappresenta una proprietà.
}
\noindent
Grazie all'Aritmetizzazione delle MdT 
\[
  X \subseteq \mathbb{N} \implies \text{ proprietà di programmi (MdT)}
\]
dove $\mathbb{N} =$ essere un programma e $\varnothing$ = non essere un programma. 
Quante proprietà esistono? Quante i sottoinsiemi dei numeri naturali ($|\mathcal{P}(\mathbb{N}|$))ovvero \textit{non numerabili}.
\ex{}
{
  \[
    \{P \in MdT \; | \; P \text{ ha } 50 \text{ righe di codice }\}
  \]
  Insieme di programmi che hanno 50 righe di codice.
  \[
    \{P \in MdT \; | \; \forall x \in \mathbb{N} \; . \; P(x) = 5\} 
  \]
  Insieme di programmi che calcolano la funzione costante 5.
  Sono due casi diversi perché 
  \begin{itemize}
    \item Nel primo, posso decidere se un programma ha 50 righe di codice contando le righe.
    \item Nel secondo, non posso decidere se un programma calcola la funzione costante 5 perché
    dovrei testare tutti gli input possibili.
  \end{itemize}
}

\dfn{Proprietà estensionali}
{
  Prendiamo $\Pi \subseteq \mathbb{N}$, $\Pi$ è detto estensionale 
  se $\forall x.y \in \mathbb{N}$:

  \[
    \varphi_x = \varphi_y \implies (x \in \Pi \iff y \in \Pi)
  \]
  Dove $\varphi_x$ è la funzione calcolata dalla MdT di indice $x$ (dal programma di codice).
}
\noindent
Ci sono due condizioni: 
\begin{enumerate}
  \item Se $x$ soddisfa la proprietà $\Pi$ ($x \in \Pi$)
  \item Se $x$ e $y$ calcolano la stessa funzione ($\varphi_x = \varphi_y$)
\end{enumerate}
Allora anche $y$ soddisfa la proprietà $\Pi$ ($y \in \Pi$).
\ex{}
{
  Se $P \in \Pi_1$ allora $P$ ha 50 righe di codice. 
  \[P' = P; step\]
  $P'$ ha 51 righe di codice. 
  $P$ e $P'$ calcolano la stessa funzione:
  \[P \in \Pi_1 \wedge P' \notin \Pi_1\]
  Quindi $\Pi_1$ non è estensionale.
}
\ex{}
{
  Presi $\varnothing, \mathbb{N}$ sono estensionali banali.
  Prendiamo allora:
  \[
    \{x \; | \; \forall y \; . \; \varphi_x(y) = y\}
  \]
  prendiamo $z \in \Pi$ per definizione di $\Pi$, $\varphi_z(y) = y, \forall y$.
  Prendiamo $w$ tale che $\varphi_w = \varphi_z$.
  \[
    \forall y. \varphi_w(y) = \varphi_z(y) = y \stackrel{def}{\implies} w \in \Pi
  \]
}
\ex{}
{
  Prendiamo: 
  \[
    \{x \; | \; \varphi_x \text{ definitivamente } \ge 3\}
  \]
  Vuol dire che da un certo punto in poi, per ogni input $y$ la funzione restituisce un valore $\ge 3$.
  Prendiamo $z \in \Pi$ dove per definizione di $\Pi$ $\varphi_z$ è definitivamente $\ge 3$ ovvero 
  $\exists m . \forall n \ge m. \varphi_z(n) \ge 3$.
  Prendiamo $w$ tale che $\varphi_w = \varphi_z$.
  \[
    \forall n \; . \; \varphi_w(n) = \varphi_z(n) \stackrel{def}{\implies} \forall n \ge m \; . \;  \varphi_w(n) = \varphi_z(n) \ge 3 \stackrel{def}{\implies} w \in \Pi
  \]  
}

\thm{Teorema di Rice}
{
  Sia $\Pi$ una proprietà estensionale. 
  $\Pi$ è ricorsiva se e solo se non è banale.
}
\begin{proof}
$(\Leftarrow$)\\
$\Pi$ è una proprietà banale $\implies \Pi = \varnothing$ o $\Pi = \mathbb{N}$ 
sono entrambi ricorsivi. 
\end{proof}
\begin{proof}
$(\Rightarrow)$ $\Pi$ è ric. $\implies$ $\Pi$ è banale\\
Prendiamo $\Pi$ ricorsivo. 
Vuol dire che esiste $g$ totale ricorsiva (caratteristica)
\[
  g(x) = \begin{cases}
    1 & x \in \Pi\\
    0 & x \notin \Pi
  \end{cases}
\]
Supponiamo per assurdo che $\Pi$ non sia banale (ovvero $\Pi \neq \varnothing$ e $\Pi \neq \mathbb{N}$).
Dove:
\begin{itemize}
  \item $\exists a \in \mathbb{N} . a \in \Pi \equiv \Pi \neq \varnothing$
  \item $\exists b \in \mathbb{N} . b \notin \Pi \neq \mathbb{N}$
\end{itemize}
Costruiamo $h: \mathbb{N} \rightarrow \mathbb{N}$ totale ricorsiva:
\[
  h(x) = \begin{cases}
    a & \text{se } g(x) = 0 \; (x \notin \Pi)\\
    b & \text{se } g(x) = 1 \; (x \in \Pi)
  \end{cases}
\]
Per il teorema di ricorsione di Kleene, allora $h$ totale ricorsiva poiché $g$ è totale ricorsiva.
Il teorema di Kleene diceva:
\[
  \exists n_0 . \varphi_{n_0} = h(n_0)
\]
Ci domandiamo dove sia $n_0$ rispetto a $\Pi$:
Se $n_0 \in \Pi$ ($\varphi_{n_0} = \varphi_{h(n_0)}$) $\Pi$ 
è estensionale quindi: 
\[
  h_{n_0} \in \Pi 
\] 
ma $h(n_0) = b \notin \Pi$ perché $n_0 \in \Pi$.
Se $n_0 \implies h(n_0) \notin \Pi$ sempre per estensionalità 
ma $h(n_0) = a \in \Pi$ perché $n_0 \notin \Pi$.
Quindi $\Pi$ deve essere $\varnothing$ o $\mathbb{N}$. $\quad \quad \square$  
\end{proof}
\ex{Alcuni esempi}
{
  Supponiamo di prendere le seguenti proprietà: 
  \[
    \{x \; | \; \forall y. \varphi_x(y) \downarrow \}
  \] 
  è estensionale perché dipende solo dalla funzione calcolata.
  \[ 
    \{ P \; | \; P \text{ contine elameno un ciclo while} \}
  \]
  Non è estensionale perché dipende dal codice.
  \[
    \{x \; | \; x \text{ ha dimensione } \ge 1000\} 
  \]
  Non è estensionale perché dipende dal codice.
  \[ 
    \{ P \; | \; P \text{ soddisfa confidenzialità} \}
  \]
  Estensionale perché dipende solo dalla funzione calcolata.
  \[ 
    \{ x \; | \; \varphi_x(x) \downarrow \} = K 
  \]
  L'insieme $K$ non è ricorsivo.
  Prendiamo $z \in \Pi$ tale che $\varphi_z(z) \downarrow$.
  Allora $w . \varphi_w = \varphi_z \implies \varphi_w(z) = \varphi_z(z)\downarrow$.
  Per poter dire che $w \in \Pi$ noi dovremmo avere $\varphi_w(w) \downarrow$.
  Quindi $\Pi$ \textbf{NON} è estensionale.
}
