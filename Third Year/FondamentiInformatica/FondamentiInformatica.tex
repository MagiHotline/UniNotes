...
\dfn{Funzioni parziali ricorsive}
{
  ...
  definiamo:
  \[\phi(x_1,\dots,x_n) = \underbrace{\mu z . (f(x_1, \dots,x_n) = 0)}_{\text{Cerca il più piccolo valore z tale che $f(x_1,\dots,x_n) = 0$ Se z non esiste $\phi(x_1, \dots,x_n) \uparrow$}}\]
  Questa operazione si chiama \textbf{minimizzazione}.
}
\noindent
Scriviamo l'algoritmo per calcolare $\phi$:
\begin{lstlisting}
  input(x_1, ..., x_n)
  z := 0
...
\begin{itemize}
  ...
  \[\phi(x_1, \dots,x_n) = \mu z.(f(x_1,\dots,x_n) = 0)\]
  Costruiamo $M_{\phi}$:
  \[
    M_{\phi} = \begin{cases}
      input(x_1, \dots, x_n)\\
...
\[
  \phi_M (x) = P_0 ( \mu t . (q_f - P_q(t, q_0, x, x \text{ mod } 2 ,0) = 0), q_0, x \text{ div } 2,x \text{ mod } 2, 0)
\]
...
$\phi$ è una funzione parziale ricorsiva se $\exists M$ MdT 
che calcola $\phi$ se e solo se $\exists$ un algoritmo che calcola $\phi$.
