...
% Lezione 4: Soluzione esercizi extra 
...
% converto in scala di grigi prima di effettuare la cross correlazione
Ibw = rgb2gray(I);
templatebw = rgb2gray(template);

% si calcola la cross correlazione normalizzata
% Attenzione all'ordine
% C = normxcorr2(TEMPLATE,A)
cc = normxcorr2(templatebw,Ibw);

% coordinate del massimo
maxcc = max(abs(cc(:))); % massimo
[r,c] = find(cc == maxcc); % coordinate del massimo

% la posizione 1,1 del kernel si trova
% Riga: r-R2+1
% Colonna: c-C2+1
[R2,C2] = size(templatebw);
...
% immagine originale a toni di grigio sui canali r,g,b, attenuata
resI(:,:,1) = Ibw*0.6;
resI(:,:,2) = Ibw*0.6;
resI(:,:,3) = Ibw*0.6;
% copio il template nella posizione corretta
resI(corr_offset(1):corr_offset(1)+R2-1,corr_offset(2):corr_offset(2)+C2-1,:) = template;
figure, imshow(resI)
