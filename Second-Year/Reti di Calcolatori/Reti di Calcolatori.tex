...
\end{examplebox}
\noindent
Altre informazioni sulla rete (dato un indirizzo IP si possono ottenere con il calcolo "\textit{whois}".
\\\\
...
Ciascuno dei due processi, ossia quello in lato client e quello in lato server, possono terminare indipendentemente la connessione TCP quando non hanno più dati da trasmettere. I casi di chiusura sono due:
\begin{itemize}
  \item Chiusura unilaterale (visto da client):
  \begin{enumerate}
    \item Il client invia un segmento al server con flag FIN = 1.
    \item Il server risponde inviando un segmento con flag ACK = 1 e si chiude la connes-
    sione lato client.
  \end{enumerate}
  \item Chiusura simultanea (visto da client):
  \begin{enumerate}
    \item Il client invia un segmento al server con flag FIN = 1.
    \item Il server risponde inviando un segmento con flag ACK = 1 e FIN = 1.
    \item Il client risponde invia un segmento con flag ACK = 1. La connessione si chiude
    sia lato client sia server.
  \end{enumerate}
\end{itemize}
Analogamente quando il server vuole chiudere la connessione invia gli stessi messaggi, ma
in direzione inversa. Alla chiusura della connessione ognuno dealloca le risorse utilizzate.
\\
...
di trasmissione chiamata MTU (Maximum Transmission Unit). \\
...
\begin{itemize}
  ...
  \item \textbf{Flag} $\rightarrow$ 3 bit: \begin{itemize}
    \item Bit "M" 
    \begin{itemize}
      \item 0 se il pacchetto non è stato frammentato oppure se è l'ultimo frammento
...
Un vantaggio del NAT è quello di limitare l'utilizzo agli indirizzi IP pubblici ad un
 1 solo IP pubblico, per un intera rete privata e quindi sopperire alla mancanza di indirizzi IP.


\subsection{Contattare utenti all'interno di una rete privata}

Tuttavia uno degli \textbf{svantaggi} del NAT è che \textit{non è possibile contattare un utente all'interno di una rete privata da un utente esterno}.
Una rete pubblica non può inviare connessioni ad una rete privata perché è composta da IP privati. 
Utilizzare l'IP dell'interfaccia del router non serve a niente perché il tuo intento è paralre direttamente con gli host della rete e il router non sa tradurre una connessione in arrivo, ma solo
solo una connessione in uscita perché conosce la destinazione.
Di conseguenza la il modo migliore per gestire la comunicazione tra una rete privata e una rete pubblica è gestirla a livello applicativo, creando una specie di "ponte" tra le due reti.
\\
Ogni applicazione ha una soluzione diversa per contattare un utente all'interno di una rete, ad esempio, Zoom ha come soluzione l'indirizzo email, Whatsapp ha come soluzione il numero di telefono, ecc.
I server directory tengono memorizzati questi ID Univoci insieme all'indirizzo IP attuale dell'utente. Quando un utente vuole contattare un altro utente (sulla stessa applicazione) chiede al server directory l'indirizzo IP dell'utente destinatario.
\begin{itemize}
  \item Se l'indirizzo IP è pubblico, lo contatta direttamente
  \item Se l'indirizzo IP è privato, si appoggia ad un altro server chiamato "Relay"
\end{itemize}
Se l'IP destinatario è \textbf{pubblico} ci si appoggia al server directory per inviare dei messaggi per stabilire la connessione diretta tra i due utenti
sfruttando l'IP pubblico. 
\\
Se l'IP destinatario è \textbf{privato} entrambi gli untenti aprono una connnessione con il server relay (che fungerà da intermediate) e i pacchetti passeranno attraverso il server relay per 
permettere la comunicazione tra i due utenti.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[scale=0.8]
    ...
    \draw (0,3) rectangle ++(3,1) node[midway] {Trasporto};
    \draw (0,2) rectangle ++(3,1) node[midway] {Rete};
    \draw (0,1) rectangle ++(3,1) node[midway,align=center]{\colorbox{blue!30!white}{Data Link}};
    \draw (0,0) rectangle ++(3,1) node[midway] {Fisico};
  \end{tikzpicture}
\end{figure}

Ogni tecnologia di rete ha il proprio protocollo di livello data-link. Questo protocollo è responsabile della trasmissione dei dati tra due nodi adiacenti.
Lo scopo del livello data link è gestire le problematiche associate ala trasmissione nel singolo hop. 
L'header del livello 2 dipende dalla tecnologia utilizzata. Un esempio è il WiFi (IEEE 802.11) o Ethernet (IEEE 802.3) o Fibra ottica (SDH)

\subsection{Problematiche legate alla trasmissione}

\begin{itemize}
  \item Gestione del mezzo condiviso
  \item Delimitazione delle trame
\end{itemize}

\subsubsection{Gestione del mezzo condiviso}

Come possiamo regolare l'accesso al mezzo condiviso, in modo da minimizzare le collisioni (più utenti trasmetto contemporaneamente) e massimizzare l'utilizzo del mezzo? 
Sono due metodi distinti ma allo stesso tempo importanti.

\vspace{1em}
\noindent
\textbf{Metodologie dell'allocazione del canale:}
\begin{itemize}
  \item \textbf{\underline{Allocazione statica}}, risorse suddivise staticamente tra i partecipanti, come TDM, Time Division Multiplex() è un esempio di allocazione statica che divide il tempo in slot temporali e assegna uno slot ad ognuno degli utenti.
  Tuttavia non è un buon utilizzo delle risorse perché può capitare che un utente non utilizzi il suo slot temporale e quindi delle risorse vanno sprecate
  \item \textbf{\underline{Allocazione Dinamica}}
  \begin{itemize}
    \item A turno (Token Ring), viene passato un token tra gli utenti e solo chi ha il token può trasmettere. Ogni utente può avere il token per un tempo massimo (100ms), tuttavia se finisce di trasmettere prima della scadenza del token lo passa al prossimo utente.
    Questa è una tecnica che non fa sprecare risorse perché se non ha niente da trasmettere passa direttamente il token. Il problema è che devo definire un ordine di trasmissione in cui viene passato il token.
    \item A contesa 
    \begin{itemize}
      \item Aloha
      \item Slotted Aloha
      \item CSMA (Base e varianti)
    \end{itemize}
  \end{itemize}
\end{itemize}
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item[$\rightarrow$] Li trasmette
  \end{itemize}
  \item Mentre trasmette, la stazione ascolta il canale
  \begin{itemize}
    \item[$\rightarrow$] Se c'è collisione (potenza rilevata > potenza trasmessa) 
    la stazione estra un tempo casuale, aspetta tale tempo e torna al punto 1
  \end{itemize}
\end{enumerate}
\begin{examplebox}{Esempio}
  Un esempio del protocollo tra due staizoni A e B:
  \textbf{DISEGNO} 
  \\
  La scelta di un tempo canale per la ritrasmissione serve per la desincronizzazione le stazioni.
  Il protocollo definisce una costante $M$ che è molto maggiore del tempo $T$ (tempo di trama (tempo che impiego a trasmettere la trama)) e il tempo causale è scelto uniformemente tra 0 e $n$.
  E in caso di collisioni consecutive, $M$ viene raddoppiato.
\end{examplebox}
Periodo di vulnerabilità: intervallo di tempo in cui una trama può subire collisioni dipende dall'algoritmo.
Per Aloha assumiamo  che il tempo di trama sia fisso o uguale a $T$ \(\left(T = \frac{B}{v}\right)\) (B = lunghezza trama, v = velocità di trasmissione)
\subsubsection{Slotted Aloha}

Introduce un piccolo meccanismo ovvero la \textit{sincronizzazione}: il tempo è suddiviso
  in intervalli di durata $T$ etutte le stazioni sono sincronizzate.
\begin{enumerate}
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item Allora trasmette la trama
    \item Altrimenti, continua ad ascoltare fino a quando il canale si libera e poi trasmette
  \end{itemize}
  ...
  \end{itemize}
\end{enumerate}
Questa variante del CSMA si chiama \textbf{persistent}.
La variante "Non Persistent" invece cambia il punto 2 in questo modo:
\begin{enumerate}
  \item[2. ] Se il canale è libero
  \begin{itemize}
    \item trasmette
    \item Altrimenti, estrae un tempo casuale e torna al punto 1
  \end{itemize}
\end{enumerate}
...
\begin{itemize}
  \item Gli indirizzi IP sono utilizzati per instradare i pacchetti tra i nodi
  \item Gli indirizzi MAC sono utilizzati per instradare i pacchetti tra i nodi adiacenti
\end{itemize}
Il livello due è necessario per trasmettere i dati in una connessione indiretta. 

\subsection{Indirizzi MAC e ARP (Address Resolution Protocol)}

Come fa un host a conoscere gli indirizzi MAC degli altri host (della propria rete o del router)?
Qui entra in gioco il protocollo ARP (Address Resolution Protocol). Lo scopo del protocollo ARP è quello di \textit{mappare un indirizzo IP con un indirizzo MAC.}
\begin{enumerate}
  \item L'host manda in broadcast una richiesta ARP (Broadcast di livello 2)
  La richiesta ARP contiene delle informazioni:
  \begin{itemize}
    \item Codice identificativo del messaggio ARP (Request o Reply)
    \item Indirizzo IP per il quale si chiede il corrispondente indirizzo Hardware
  \end{itemize}
  \item Solo chi possiede l'indirizzo IP indicato nella richiesta risponde direttamente a chi ha fatto richiesta.
  \begin{itemize}
    \item La risposta contiene l'indirizzo MAC cercato
    \item La risposta ARP è unicast
  \end{itemize}
\end{enumerate}

Le risposte ARP ottenute vengono memorizzate in una tabella ARP.
Questa tabella contiene le coppie IP-MAC e ha un tempo di vita (TTL) di 20 minuti.

\subsection{LAN Estese}

Esistono degli apparati di rete di livello 2 che sostiuiscono il cavo coassiale nelle LAN:
Come per esempio lo \textbf{switch}. Lo switch è un apparato di rete che permette di collegare più host tra loro.
Gli switch hanno una tabella contenente l'indirizzo MAC e la porta dello switch corrispondente a cui collegato l'host. 
Il pacchetto \textit{non viene} modificato dallo switch, ma solo quando passa nel livello di Rete andrò a scartare il livello e appenderò
un ulteriore livello 2. Esistono diverse configurazioni: gli switch si possono collegare tra di loro:

\subsubsection{LAN Wireless 802.11}

Le reti wireless sono molto simili alle reti cablate, ma con alcune differenze:
\begin{itemize}
  \item Il mezzo trasmissivo è l'aria
  \item Il canale è condiviso
  \item La trasmissione è half-duplex
  \item La trasmissione è broadcast
  \item La trasmissione è soggetta a interferenze
\end{itemize}
Bisogna prima capire cosa sia il \textbf{Time Slot} definito come tempo di propagazione minimo tra Access Point e le stazioni associate.
La distanza massima che ci può essere tra una AP e un Host Fisico è legato da condizioni fisiche. 
Dipende dalla frequenza utilizzata $\tau << T$. 
\begin{examplebox}{Esempio}
  802.11b: \( \tau = 10 \mu s \) e \( T = 1 ms \)
\end{examplebox}
\noindent 
Definiamo 2 intervalli:
\begin{enumerate}
  \item SIFS (Short Interframe Space) $\rightarrow$ Intervallo tra due trame consecutive $\tau$ 
  \item DIFS (Distributed Interframe Space) $\rightarrow$ Intervallo tra due trame consecutive di stazioni diverse $3\tau$
\end{enumerate}
Il tempo viene slottizzatto ed è determinato dall'Access Point. 
\subsubsection{CSMA-CA (Congestion Avoidance)}
...
\begin{enumerate}
  \item Se il canale è libero
  \begin{itemize}
    \item Continuo ad ascoltare il canale per un intervallo pari a DIFS, se il canale continua ad essere libero, trasmette la trama
  \end{itemize}
  \item Se il canale è occupato (fin da subito o durante DIFS) continuo ad ascoltare il canale finché non si libera
  \item Quando si è liberato, ascolto per un ulteriore intervallo DIFS. Se il canale torna occupato, torno al punto 2
  \item Se il canale rimane libero per un intervallo DIFS:
  \begin{itemize}
    \item La stazione estra un numero causale uniformemente distribuito (tra 0 e $CW-1$) (Contention Window)),
    questo numero casuale $s$ viene interpretato come: 
    \[s = \# \text{ di slot (SIFS)}\]
    che devi attendere prima di poter trasmettere
    \item Fintantoché il canale rimane libero la stazione decrementa $s$.
    \item Se $s = 0$ trasmette la trama
    \item Se il canale torna occupato prima che $s$ diventi $0$ congelo il countdown, torno al punto 2, 
    ma al punto 4 utilizzerò il numero $s$ congelato.
  \end{itemize}
  \item Se c'è collisione:
  \begin{itemize}
    \item Interrompo la trasmissione
    \item Estrae un numero casuale e torna al punto 1
  \end{itemize}
\end{enumerate}
Nelle WLAN c'è la possibilità di attivare il riscontro al livello 2.
Il riscontro deve essere mandato dopo un SIFS.
\subsection{Problema del terminale nascosto}
Il problema del terminale nascosto è specifico delle reti wireless.
Una trasmissione wireless è omnidirezionale quindi la potenza trasmessa
si distribuisce sulla superficie di una sfera. Una volta aver superato il range di trasmissione, la potenza del segnale si riduce, di conseguenza
il segnale non è più ricostruibile.
...
\begin{examplebox}{Esempio}
  ...
  \begin{itemize}
    \item La stazione B ascolta il canale e vede che è libero
    \item L'AP percepisce due segnali $\rightarrow$ collisione e nessuna 
    delle due stazioni si accorge che c'è stata una collisione
  \end{itemize}

  Ci sono tre possibili soluzioni:
  \begin{enumerate}
    \item ACK di Livello di 2
    \item Limitare lo spazio di responsabilità dell'access point
    \item Soluzione protocollare, introduzione di due messaggi RTS e CTS ovvero Request to Send e Clear to Send.
    L'uso di RTS e CTS permette di risparmiare energia, spengo il circuito di ricezione dopo aver ricevuto un CTS. 
    Se non c'è RTS e CTS c'è comunque la possibilità di risparmia energia sul canale: ogni volta che c'è una trama e guardo il MAC address destinatario
    e il campo della lunghezza della trama. Se non sono la destinazione, spengo il canale di ricezione per il tempo necessario per la ricezione della trama.
