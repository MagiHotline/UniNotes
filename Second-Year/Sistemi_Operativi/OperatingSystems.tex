...
\textcolor{green!50!black}{
1.2 Alternative code for the run method of the PrintNum class:
}
\begin{minted}
{java}
...
\end{minted}
\noindent
With this version, the results are consistent. The \texttt{PrintNum} thread will print numbers from 1 to 50, and the \texttt{PrintChar} thread will print 'c' 40 times. The \texttt{thread4.join()} method ensures that the main thread waits for the \texttt{PrintChar} thread to finish before continuing.\\
\textcolor{green!50!black}{
1.3 Alternative code for the run method of the PrintNum class:
}
\begin{minted}{java}
@Override /** Tell the thread how to run */
public void run() {
...
\end{minted}
\noindent   
This time with the yield method, it doesn't vary anymore and the results are consistent. The \texttt{Thread.yield()} method suggests to the thread scheduler that the current thread is willing to yield its current use of the CPU. 
Now we try yet another alternative code for the run method of the PrintNum class:
\begin{minted}{java}
public void run() {	
	try{
		for (int i = 1; i <= lastNum; i++) {
...
\end{minted}
With this version, we introduce the \texttt{Thread.sleep()} method, which in the first example pauses the thread for 1 millisecond after printing numbers up to 50. The results are still consistent, but the execution time may vary slightly due to the sleep duration. In fact:
\begin{itemize}
    \item 1 ms sleep is not enough to see a significant difference in execution time.
    \item trying to increase the sleep time to 10 ms or more will show a more noticeable difference in execution time. After the 50th number you can see the printing of the numbers.
\end{itemize}
Now trying to change the priority inserting this code:
\begin{minted}{bash}
thread1.start();
thread2.start();
...
thread3.start();    
\end{minted}
We can notice the change in the execution. In fact, the program will try to give more CPU time to the thread with the highest priority, which is \texttt{thread3}. This can lead to a more consistent execution time for that thread, but it may also starve other threads of CPU time, especially if they have lower priority.\\
