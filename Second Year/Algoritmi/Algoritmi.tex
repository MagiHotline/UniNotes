...
\begin{proof}
  \textbf{Ci sono $2^k$ nodi}: Supponiamo per induzione che in albero di dimensione $k$ ci siano $2^k$ nodi.
...
Supponiamo di voler fare l'unione di due heap binomiali $H_1$ e $H_2$. TODO TODO
...
Se voglio rimuovere un nodo (magari il più piccolo) dovrei eliminare la radice.
L'idea è quella di rimuovere il nodo e poi unire tutti gli alberi binomiali che sono stati divisi.
Rimuovere la radice si può fare in tempo $O(\log n)$.

\subsubsection{Diminuzione di una chiave e rimuovere un elemento arbitrario}

Se voglio dimunuire una chiave in un nodo devo stare attento a non violare la proprietà di heap.
Se la chiave diminuita è minore della chiave del padre, allora devo scambiare il nodo con il padre e così via fino a che non violo la proprietà di heap.
La complessità di questa operazione è $O(\log n)$.
Ridurre una chiave potrebbe essere utili in diversi campi. Ci sono molte operazioni molto potenti che però portano a complessità maggiori.
Per \textbf{rimuovere un nodo arbitrario} grazie alla diminuzione della chiave, posso portarla fino alla radice in modo che poi rimuovere la radice sia un'operazione $O(\log n)$.
Quindi in tempo logaritmico con una Heap posso:
\begin{itemize}
  \item Inserire un nodo
  \item Trovare il minimo
  \item Unire due Heap Binomiali
  \item Rimuovere un nodo
  \item Diminuire una chiave
  \item Rimuovere un nodo arbitrario
\end{itemize}
Cosa non posso fare in tempo logaritmico?
\begin{itemize}
  \item Trovare il massimo
  \item Cercare un elemento arbitrario
\end{itemize}
\subsubsection{Dividere un heap binomiale}
Se io volessi dividere un heap grande in due heap più piccoli mantenendo la differenza
al più di uno bisogna mantenere da parte il nodo di dimensione 0 e dividere il resto. il
nodo da parte verrà poi riaggiunto ad una delle due liste utilizzando un unione.

\subsubsection{Trovare il mediano}

Essendo l'array non ordinato, trovare il mediano è un'operazione che richiede tempo lineare.
La chiave è trasformare i dati nell'array in un heap binomiale e poi procedere per trovare il mediano. Poiché l'heap binomiale non supporta direttamente l'accesso al mediano, sono necessari alcuni passaggi aggiuntivi.
