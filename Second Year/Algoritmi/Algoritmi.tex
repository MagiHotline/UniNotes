...
I problemi di flusso massimo sono un argomento molto importante in informatica, perché sono alla base di molti algoritmi e applicazioni pratiche.
Il problema del flusso massimo è quello di trovare il flusso massimo in un grafo diretto con capacità sugli archi. 
La rete di flusso è un grafo diretto $G = (V, E)$ con una funzione di capacità $c : V \times V \rightarrow R^+$ e due nodi speciali: la sorgente $s$ e il pozzo $t$.
La capacità deve essere tale che $c(u,v) \ge 0$ per ogni arco $(u,v) \in E$ e $c(u,v) = 0$ se $(u,v) \notin E$.
...
\begin{figure}[H]
  ...
  \draw[->] (S) -- node[above]{$16$} (A);
  \draw[->] (S) -- node[below]{$13$} (C);
  \path[->] (C) edge [bend left =25] node[left =0.1 cm] {$10$} (A);
  \path[->] (A) edge [bend left =25] node[left =0.1 cm] {$4$} (C);
  \draw[->] (C) -- node[below]{$14$} (D);
  \draw[->] (A) -- node[above]{$12$} (B);
  \draw[->] (B) -- node[above]{$9$} (C);
  \draw[->] (D) -- node[right]{$7$} (B);
  \draw[->] (B) -- node[above]{$20$} (T);
  \draw[->] (D) -- node[below]{$4$} (T); 
\end{tikzpicture}
\end{figure}
\noindent
Dobbiamo cercare di trovare un flusso massimo da $S$ a $T$. Non posso utilizzare un approccio greedy dove mando tutto il flusso possibile da $S$ a $T$ perché non è detto che sia ottimo.
\noindent
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[auto, node distance =2 cm and 3cm ,on grid ,
  ...
  \draw[->] (S) -- node[above]{$11/16$} (A);
  \draw[->] (S) -- node[below]{$8/13$} (C);
  \path[->] (C) edge [bend left =25] node[left =0.1 cm] {$0/10$} (A);
  \path[->] (A) edge [bend left =25] node[left =0.1 cm] {$1/4$} (C);
  \draw[->] (C) -- node[below]{$11/14$} (D);
  \draw[->] (A) -- node[above]{$12$} (B);
  \draw[->] (B) -- node[above]{$4/9$} (C);
  \draw[->] (D) -- node[right]{$7$} (B);
  \draw[->] (B) -- node[above]{$20$} (T);
  \draw[->] (D) -- node[below]{$4$} (T); 
\end{tikzpicture}
...
\begin{figure}[H]
  ...
  \path[->] (A) edge [bend left =25] node[left =0.1 cm] {$1/4$} (C);
  \draw[->] (C) -- node[below]{$11/14$} (D);
  \draw[->] (A) -- node[above]{$12/12$} (B);
  \draw[->] (B) -- node[above]{$4/9$} (C);
  \draw[->] (D) -- node[right]{$7/7$} (B);
  \draw[->] (B) -- node[above]{$15/20$} (T);
  \draw[->] (D) -- node[below]{$4/4$} (T); 
\end{tikzpicture}
\end{figure}
\noindent
