...
\dfn{}{
    Una superficie in forma parametrica è una funzioen $r: A \subset \mathbb{R}^2 \rightarrow \mathbb{R}^3$ definita da
...
che sta tra il grafico $f$ e il piano $xy$ e che è delimitata dalla superficie.
\dfn{}
{
    Si dice che la funziene $f : [a,b] \times [c,d] \rightarrow \mathbb{R}$ è integrabile nel rettangolo $R = [a,b] \times [c,d]$ se esiste il limite finito
    $lim_{n \rightarrow \infty} s_n$ data da (7.1.1) e inoltre tale limite non dipende dalla scelta dei punti $p_{hk}$.
    In tal caso tale limite si dice \textbf{integrale doppio} di $f$ su $R$ e si indica con:
    \[\iint_{R} f(x,y) dxdy = \lim_{n \rightarrow \infty} \sum_{h,k=1}^{\infty} |I_{hk}| f(p_{hk}) \]
}
\noindent
\begin{figure}[H]
\centering
\begin{tikzpicture}
  \begin{axis}[
    % PROSPETTIVA 3D
    view={60}{30},                  % angolazione (in gradi) del punto di vista: {azimuth}{elevation}
    axis lines=center,              % assi x,y,z tutti e tre mostrati
    xlabel={$x$}, ylabel={$y$}, zlabel={$z$},
    xmin=0, xmax=1.2,               % estendere leggermente oltre 1 per spazi vuoti
    ymin=0, ymax=1.2,
    zmin=0, zmax=1,                 % qui z max sarà xy al massimo uguale a 1
    % griglia leggera (opzionale)
    grid=both,
    grid style={gray!30, line width=0.3pt},
    % colormap per la superficie
    colormap/cool,
  ]
    %% 1) Superficie z = x*y
    \addplot3[
      ...
      samples=21, samples y=21,      % campionatura più fitta per avere mesh regolare
    ]
    {x*y};

    %% 2) Base del dominio R nel piano z=0
    \addplot3[
      draw=none,
      fill=blue!20,
      opacity=0.4,
      domain=0:1, y domain=0:1,
      samples=2, samples y=2,        % basta il rettangolo alle 4 estremità
    ]
    ({x}, {y}, {0});

    %% 3) Alcuni segmenti verticali (i "prismi") per illustrare il valore f(x,y)
    %    Scegliamo 4 punti campione nel reticolo: (0.25,0.25), (0.75,0.25), (0.25,0.75), (0.75,0.75)
    \addplot3[line width=0.5pt] coordinates { (0.25,0.25,0) (0.25,0.25,0.25*0.25) };
    \addplot3[line width=0.5pt] coordinates { (0.75,0.25,0) (0.75,0.25,0.75*0.25) };
    \addplot3[line width=0.5pt] coordinates { (0.25,0.75,0) (0.25,0.75,0.25*0.75) };
    \addplot3[line width=0.5pt] coordinates { (0.75,0.75,0) (0.75,0.75,0.75*0.75) };

    %% 4) Contorno della superficie in trasparenza per maggior chiarezza opzionale
    \addplot3[
      surf,
      opacity=0,               % solo il contorno mesh senza riempimento
      shader=flat,
      draw=black!40,           % colore del contorno
      domain=0:1, y domain=0:1,
  ...
  \end{axis}
\end{tikzpicture}    
\caption{$\iint f(x,y)dA$ su $R = [0,1] \times [0,1]$ con $f(x,y) = xy$}
\end{figure}
\ex{}
{
...
\thm{Di riduzione dell'integrale doppio su un rettangolo a integrali iterati}
{
...
\thm{Funzioni a variabili separate}
{
